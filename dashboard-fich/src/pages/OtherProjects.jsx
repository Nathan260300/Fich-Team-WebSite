...
function ProjectModal({ project, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    let err;
    if (isNew) {
      const { data: maxRow } = await supabase.from('other_projects').select('sort_order').order('sort_order', { ascending: false }).limit(1).single();
      payload.sort_order = (maxRow?.sort_order ?? -1) + 1;
      ({ error: err } = await supabase.from('other_projects').insert(payload));
    } else {
      ({ error: err } = await supabase.from('other_projects').update(payload).eq('id', project.id));
    }
    setSaving(false);
        ...
        <div className={s.modalBody}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className={s.field}>
              ...
              <input value={form.id} onChange={e => set('id', e.target.value)} placeholder="ex: build-spawn" disabled={!isNew} />
            </div>
            <div className={s.field}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div className={s.field}>
              ...
              <input value={form.date_label} onChange={e => set('date_label', e.target.value)} placeholder="Ex: Mars 2025" />
            </div>
          </div>
...
export default function OtherProjects() {
  ...
  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from('other_projects').select('*').order('sort_order');
    setItems(data ?? []);
    setLoading(false);
  };
  ...
  const handleDelete = async (id) => {
    ...
    await supabase.from('other_projects').delete().eq('id', id);
    load();
  };

  return (
    <div>
      <PageHeader title="Autres projets" desc="Gérer la timeline des autres projets." action={<button className={s.btnPrimary} onClick={() => setModal({})}>+ Ajouter</button>} />
